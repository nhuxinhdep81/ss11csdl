...
(
    ...
    out cusId int,
    out TotalBalance int
)
begin
select c.CustomerID, sum(a.Balance) into cusID, TotalBalance
from Customers c 
...
call GetTotalBalance(1,@cusID,@TotalBalance);
select @cusID as CustomerId,@TotalBalance as TotalBalance;
...
call IncreaseEmployeeSalary(1, @newSalary);
...
begin
	select CustomerID,FullName,DateOfBirth,Address,PhoneNumber,Email
    from Customers
    where PhoneNumber = phone_in;
end //
delimiter //
