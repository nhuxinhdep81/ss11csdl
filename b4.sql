...
begin
    ...
    from employees 
	where employeeId = employ_id_in;
    
    if NewSalary < 20000000 then set NewSalary = NewSalary * 1.1 ;
    else set NewSalary = NewSalary * 1.05;
	end if;
...
(
    ...
    out total_loan decimal(15,2)
)
begin
	select sum(l.LoanAmount) into total_loan
	from Loans l where l.CustomerID = CustomerID_in
    group by CustomerID;

    if total_loan is null 
    then set total_loan = 0;
    end if;
end //
delimiter //
...
declare balance_now decimal(15,2);

select Balance into balance_now
from Accounts 
where AccountId = AccountID_in;

if balance_now < 1000000 then 
	delete from Accounts
	where AccountId = AccountID_in ;
        select 'xoa tai khoan thanh cong!' as Messaga;
	else 
...
(
	in account_id_in_deposit int,
    in account_id_out_received int,
    inout transfer_amount decimal(15,2)
)
begin
declare money_id_in decimal(15,2);
declare money_id_out decimal(15,2);

select Balance into money_id_in
from Accounts
where AccountID = account_id_in_deposit; 

select Balance into money_id_out
from Accounts
where AccountID = account_id_out_received; 

if money_id_in >= transfer_amount
then 
 update Accounts
...
delimiter //
set @transfer_amount = 10000000000;
call TransferMoney(3,1, @transfer_amount);
